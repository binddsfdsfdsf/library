...
class StartPageUSER(CTkFrame):
    def __init__(self, master):
        ...
        CTkButton(master=self.sidebar_frame, text="Books", fg_color="#fff", font=("Arial Bold", 14), text_color="#765827", anchor="w",hover_color="#C8AE7D",command=self.to_now).pack(anchor="center", padx=5, pady=(16, 0))
    ...
    def to_now(self):
        self.master.switch_frame(StartPageUSER)
    def to_search(self):
...
class ToplevelWindowBor_boo(CTkToplevel):
    ...
    def Borrow_Book1(self):
        ...
        print(self.text)
        connection=create_connection()
        self.Borrow=search_book(connection,self.text)
        print(self.text)
